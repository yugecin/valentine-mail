...
vec4 stamp(vec2 uv)
{
	...
	float y = abs(uv.y - .5);
	float x = mod(uv.x, .2);
	if (length(uv-vec2(x,y)) < .1) {
		return vec4(0.);
	}
	*/
	uv.y = ((uv.y - .5) / 1.4) + .5;
	//uv.x = mod(uv.x, .4);
	vec2 huv = mod(uv-vec2(.05,0.), .1)+.05;
	if ((uv.x < .05 || uv.x > .95 || uv.y < .2 || uv.y > .8) && length(huv - vec2(.1)) < .03) {
		return vec4(0.);
	}
...
vec3 colorHit(vec4 result, vec3 rd)
{
	...
	if (int(result.w) == MAT_PAP) {
		...
		vec2 xx = prel2(vec2(.75,.63),vec2(.95,.92),threeduv);
