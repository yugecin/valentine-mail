...
bool postmark2(vec2 uv) {
	if (uv.x < .54) {
		...
		if (w < .32) {
			...
			float u = 3.14-.5;
			if (.5 < r && r < u) {
				uv.y = mix(1.15,-.1,(w - .33)/.16);
				uv.x = mix(1.3,-.4,(r-.5)/u);
				return texture2D(tex, mix(vec2(.0,.34),vec2(.19,.41),uv)).x>.2; // belgium
			}
		}
	}
	return false;
...
vec3 colorHit(vec4 result, vec3 rd)
{
	...
	if (int(result.w) == MAT_PAP) {
		...
		xx = prel2(vec2(.55,.56),vec2(1.,.84),threeduv);
		if (xx.x > 0. && xx.y > 0. && xx.x < 1. && xx.y < 1.) {
